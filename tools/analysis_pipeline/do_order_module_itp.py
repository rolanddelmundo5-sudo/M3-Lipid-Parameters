...
# 2022.09.03 - Helgi I. Ingolfsson - read bond information from provided .itp file (has to have right @BOND line after first mention of lipid name)
#                                    format e.g. ";@BONDS NC3-PO4 PO4-GL1 GL1-GL2 GL1-D1A D1A-D2A D2A-D3A D3A-D4A D4A-C5A GL2-D1B D1B-D2B D2B-D3B D3B-D4B D4B-C5B "
...
"""
  ...
  Usage: %s <traj file> <tpr file> <initial time> <final time> <skip frames> <bilayer normal - xyz> <#lipids> <lipid type> <.itp file>

    > %s my.xtc my.tpr 5000 10000 5 0 1 0 64 DPPC lipids_all.itp
...
def computeOrderParameter(fp, trajfile, tprfile, indexfile, initial_time, final_time, traj_skip, normalx, normaly, normalz, number_of_lipids, lipid_type, itpfile, outfile):
  print(f"orderparam run: {fp}, {trajfile}, {tprfile}, {indexfile}, {initial_time}, {final_time}, {traj_skip}, {normalx}, {normaly}, {normalz}, {number_of_lipids}, {lipid_type}, {itpfile}, {outfile}")
  ...
  # Get bond list for lipid_type
  bond_names = f"No @BONDS infor for lipid {lipid_type} found in {itpfile}"
  with open(itpfile) as f:
    line = f.readline()
    while line:
      if lipid_type in line.strip():
        # Found lipid check if @BOND in comment 
        while line and len(line.strip()) > 0 and line.strip()[0] == ";":
          if "@BONDS" in line.strip():
            bond_names = " " + line.strip()[8:] + "\n"
          line = f.readline() 
        break 
      line = f.readline()
  print(f"BOND list {bond_names}")
  ...
  elif lipid_type == "POPX": bond_names = phosphatidylcholine_bond_names + "GL1-C1A C1A-D2A D2A-C3A C3A-C4A GL2-C1B C1B-C2B C2B-C3B C3B-C4B\n"
  ...
  command = "echo %s | gmx trjconv -f %s -s %s -n %s -b %i -e %i -sep -skip %i -pbc whole -o %sframe_dump_.gro > /dev/null" % (lipid_type, trajfile, tprfile, indexfile, initial_time, final_time, traj_skip, fp)
  print(command)
  ...
  bonds = []
  while True:
    filename = fp + "frame_dump_" + str(file_count) + ".gro"
  ...
  # WARNING this works with currenct lipids - it uses the names of current tails beads to deafine what is in the tail - expand this if other tail names are pressent
  '''
  ave_chain_s = 0
  ...
  '''
  # Tail name is normally x3 letters e.g. C1A D1A T1A - where 1 can is the tail bead number so 1-x and A can be A B or C indicating the tail 
  re_tailbead = re.compile('[CDT]+[0-9]+[ABC]+')  
  tailCount = 0
  avg_tail_s = 0
  boundlist = bond_names.split()
  #print(f"bonds count {len(boundlist)} oparams count {len(averaged_order_parameters)} bond list {bond_names}")
  for i in range(len(boundlist)):
    front, back = boundlist[i].split("-")
    if re_tailbead.match(back):
      #print(f"YES for {boundlist[i]} - values {averaged_order_parameters[i]}")
      avg_tail_s += abs(averaged_order_parameters[i])
      tailCount += 1
  #print(f"final sum {avg_tail_s} avg {avg_tail_s/tailCount} of {tailCount} bonds")
  avg_tail_s = avg_tail_s / tailCount
  average_txt = "Abs average order parameters for carbon chains <Sn> = %8.3f \n\n" % (avg_tail_s)
  stdout.write(average_txt)
  output.write(average_txt)
